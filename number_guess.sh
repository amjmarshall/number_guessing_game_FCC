...
PSQL="psql --username=freecodecamp --dbname=number_guess_db -t --no-align --tuples-only -c"
...
then
  ...
  echo UPDATE_BEST_GAME=$($PSQL "UPDATE users SET best_game=$NUMBER_OF_GUESSES WHERE user_id=$USER_ID")
fi
